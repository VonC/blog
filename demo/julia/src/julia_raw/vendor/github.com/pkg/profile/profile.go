...
// NameProfile complete the default name.
func NameProfile(name string) func(*Profile) {
	return func(p *Profile) {
		p.name = name
	}
}
...
// TraceProfile controls if execution tracing will be enabled. It disables any previous profiling settings.
...
// Stop stops the profile and flushes any unwritten data.
func (p *Profile) getName(name string) string {
	if p.name == "" {
		return name
	}
	split := strings.Split(name, ".")
	res := fmt.Sprintf("%s_%s.%s", split[0], p.name, split[1])
	return res
}
...
func Start(options ...func(*Profile)) interface {
	...
	switch prof.mode {
	case cpuMode:
		fn := filepath.Join(path, prof.getName("cpu.pprof"))
		f, err := os.Create(fn)
		if err != nil {
	...
	case memMode:
		fn := filepath.Join(path, prof.getName("mem.pprof"))
		f, err := os.Create(fn)
		if err != nil {
	...
	case mutexMode:
		fn := filepath.Join(path, prof.getName("mutex.pprof"))
		f, err := os.Create(fn)
		if err != nil {
	...
	case blockMode:
		fn := filepath.Join(path, prof.getName("block.pprof"))
		f, err := os.Create(fn)
		if err != nil {
	...
	case traceMode:
		fn := filepath.Join(path, prof.getName("trace.out"))
		f, err := os.Create(fn)
		if err != nil {
